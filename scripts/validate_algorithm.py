...
Compares the Adaptive Matched Filter methane detection against the NASA L2B product
on a known Permian Basin event.
"""
import earthaccess
import xarray as xr
...
def find_permian_granule():
    ...
    # Golden IDs provided by user (Aug 5 invalid, using Aug 4 confirmed)
    ...
    if l2a_results and l2b_results:
        print(f"✓ Found exact requested granules")
    ...
    # Pick the one closest to the requested time ("182042")
...
def validate_algorithm(z_threshold: float = 4.0, save_dir: str = "./algorithm_results"):
    """
    Run validation comparing AMF-based methane detection to the NASA L2B product.
    ...
    print("=" * 80)
    print("METHANE DETECTION - AMF VALIDATION")
    print("=" * 80)
    ...
    # 4. Run Adaptive Matched Filter
    print(f"\n🔬 Running Adaptive Matched Filter (threshold={z_threshold}σ)...")
    try:
        amf_detection_map, metadata = run_matched_filter(ds_l2a, z_threshold=z_threshold)
        print(f"  ✓ Complete!")
    ...
    if 'ch4_plume_complex' in ds_l2b:
        ...
        nasa_mask = np.zeros_like(amf_detection_map)
    
    n_nasa_pixels = (nasa_mask > 0).sum()
    print(f"  ✓ NASA detected {n_nasa_pixels:,} plume pixels")
    
    # 6. Extract plume locations from AMF detection map
    if 'lat' in ds_l2a and 'lon' in ds_l2a:
        lat_array = ds_l2a['lat'].values
        lon_array = ds_l2a['lon'].values
        
        from chemeye.services.methane_amf import extract_plume_locations
        plumes = extract_plume_locations(amf_detection_map, lat_array, lon_array, min_pixels=5)
        
        print(f"\n🎯 AMF Plume Detections: {len(plumes)}")
    ...
    fig.suptitle(f"AMF vs NASA: {l2a_name}", fontsize=14)
    
    # Plot 1: AMF Detection Map
    im1 = axes[0].imshow(amf_detection_map, cmap='inferno', vmin=0, vmax=10)
    axes[0].set_title(
        "Adaptive Matched Filter\n"
        f"Max: {metadata['max_z_score']:.1f}σ | Pixels: {metadata['n_detection_pixels']:,}"
    )
    ...
    # Plot 3: Overlay (AMF in Red, NASA in Blue)
    overlay = np.zeros((amf_detection_map.shape[0], amf_detection_map.shape[1], 3))
    overlay[:, :, 0] = np.clip(amf_detection_map / 10, 0, 1)  # AMF = Red
    overlay[:,:,2] = np.clip(nasa_mask / nasa_mask.max() if nasa_mask.max() > 0 else 0, 0, 1)  # NASA = Blue
    axes[2].imshow(overlay)
    axes[2].set_title("Overlay\nRed=AMF | Blue=NASA | Purple=Both")
    axes[2].set_xlabel("Crosstrack")
    
    plt.tight_layout()
    
    plot_path = Path(save_dir) / "amf_validation.png"
    ...
    results = {
        ...
        "amf_metadata": metadata,
        "nasa_pixels": int(n_nasa_pixels),
        "z_threshold": z_threshold,
        "amf_plumes": len(plumes) if 'plumes' in locals() else 0
    }
    
    json_path = Path(save_dir) / "algorithm_results.json"
    ...
    if metadata['n_detection_pixels'] > n_nasa_pixels:
        print(f"🏆 AMF FOUND MORE: {metadata['n_detection_pixels']:,} vs {n_nasa_pixels:,} pixels")
        ...
        print(f"⚠️  AMF MORE CONSERVATIVE: {metadata['n_detection_pixels']:,} vs {n_nasa_pixels:,}")
        print(f"   Try lowering threshold (currently {z_threshold}σ)")
    
    print("\nNext Steps:")
    print("  1. Inspect amf_validation.png visually")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Validate methane AMF detection")
    parser.add_argument("--threshold", type=float, default=4.0, help="Z-score threshold")
    parser.add_argument(
        "--output",
        type=str,
        default="./algorithm_results",
        help="Output directory for plots and JSON",
    )
    
    args = parser.parse_args()
    
    validate_algorithm(z_threshold=args.threshold, save_dir=args.output)
